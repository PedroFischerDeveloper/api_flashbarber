...
export default class ServiceProviderController {
    ...
    async getById(req: any, res: any) {
        const provider = await this.Model.getById(req);
        return res.status(provider.status).json({response: provider.response, message: provider.message});
    }

    async save(req: any, res: any) {
        const provider = await this.Model.save(req, res);
        return res.status(provider.status).json({response: provider.response, message: provider.message});
    }

    async update(req:any, res:any) {
        const provider = await this.Model.update(req, res);
        return res.status(provider.status).json({response: provider.response, message: provider.message});
    }
